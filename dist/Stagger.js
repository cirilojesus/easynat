...
import Animated, { useSharedValue, useAnimatedStyle, withTiming, Easing, interpolateColor, interpolate, withDelay, } from 'react-native-reanimated';
import { Box } from './Box';
const DEFAULT_DURATION = 400;
const DEFAULT_DELAY = 0;
const DEFAULT_EASING = Easing.bezier(0.4, 0, 0.2, 1);
export const Stagger = forwardRef(({ children, from = { opacity: 0, translateY: 20, scale: 0.95 }, to = { opacity: 1, translateY: 0, scale: 1 }, animateOnInit, staggerDelay = 80, delay, duration, easing, ...props }, ref) => {
    ...
    return (<Box {...props}>
                {React.Children.map(children, (child, index) => (<StaggerItem key={index} index={index} from={from} to={to} animateOnInit={animateOnInit} staggerDelay={staggerDelay} delayGlobal={delay} durationGlobal={duration} easingGlobal={easing} registerShow={(fn) => (showRefs.current[index] = fn)} registerHide={(fn) => (hideRefs.current[index] = fn)}>
                        {child}
                    </StaggerItem>))}
            </Box>);
});
const StaggerItem = ({ from, to, index, staggerDelay, animateOnInit, durationGlobal, delayGlobal, easingGlobal, registerShow, registerHide, children, }) => {
    ...
    const style = useAnimatedStyle(() => {
        ...
        const opacity = (from.opacity ?? 1) +
            (to.opacity ?? 1 - (from.opacity ?? 1)) * progress.value;
        const backgroundColor = from.backgroundColor || to.backgroundColor
            ? interpolateColor(progress.value, [0, 1], [from.backgroundColor ?? 'transparent', to.backgroundColor ?? 'transparent'])
            : undefined;
        const borderColor = from.borderColor || to.borderColor
            ? interpolateColor(progress.value, [0, 1], [from.borderColor ?? 'transparent', to.borderColor ?? 'transparent'])
            : undefined;
        const borderWidth = interpolate(progress.value, [0, 1], [from.borderWidth ?? 0, to.borderWidth ?? from.borderWidth ?? 0]);
        const borderRadius = interpolate(progress.value, [0, 1], [from.borderRadius ?? 0, to.borderRadius ?? from.borderRadius ?? 0]);
    ...
    const styleText = useAnimatedStyle(() => {
        const color = from.color || to.color
            ? interpolateColor(progress.value, [0, 1], [from.color ?? '#000', to.color ?? from.color ?? '#000'])
            : undefined;
        const letterSpacing = interpolate(progress.value, [0, 1], [from.letterSpacing ?? 0, to.letterSpacing ?? from.letterSpacing ?? 0]);
    ...
    return (<Animated.View style={style}>
            {React.cloneElement(children, { children: childsChildren })}
        </Animated.View>);
};
// import React, { forwardRef, useEffect, useImperativeHandle, useRef } from 'react';
