...
const StaggerItem = ({ from, to, index, staggerDelay, animateOnInit, durationGlobal, delayGlobal, easingGlobal, registerShow, registerHide, children, }) => {
    ...
    const animateTo = (v) => {
        const isForward = v === 1;
        const duration = (isForward ? to.duration : from.duration) ||
            durationGlobal ||
            DEFAULT_DURATION;
        const delayBase = (isForward ? to.delay : from.delay) ||
            delayGlobal ||
            DEFAULT_DELAY;
        const easing = (isForward ? to.easing : from.easing) ||
            easingGlobal ||
            DEFAULT_EASING;
    ...
    const textStyle = (0, react_native_reanimated_1.useAnimatedStyle)(() => {
    ...
    const childsChildren = react_1.default.Children.map(children.props.children, (x) => {
        if (typeof x === 'string') {
            return <react_native_reanimated_1.default.Text style={textStyle}>{x}</react_native_reanimated_1.default.Text>;
        }
        return x;
    });
...
function InternalStagger(_a, ref) {
...
exports.Stagger = (0, react_1.forwardRef)(InternalStagger);
