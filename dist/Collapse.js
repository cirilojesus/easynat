...
import Animated, { useSharedValue, useDerivedValue, useAnimatedStyle, withTiming, useAnimatedReaction, runOnJS, } from 'react-native-reanimated';
import { Box } from './Box';
import { Icon } from './Icon';
const IconAnimated = Animated.createAnimatedComponent(Icon);
export const Collapse = forwardRef(({ trigger, ...props }, ref) => {
    const height = useSharedValue(0);
    const isExpanded = useSharedValue(false);
    const [openState, setOpenState] = useState(false);
    useAnimatedReaction(() => isExpanded.value, (val) => {
        runOnJS(setOpenState)(val);
    });
    const derivedHeight = useDerivedValue(() => withTiming(isExpanded.value ? height.value : 0, { duration: 300 }));
