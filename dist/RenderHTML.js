...
export function renderHTML(html) {
    ...
    html.replace(tagRegex, (match, tagName, attrs, textContent) => {
        if (textContent) {
            current.children.push(<Text key={current.children.length}>
...
function renderNode(node) {
    ...
    switch (node.name) {
        case "div":
            return (<Box key={key} w="100%" flexWrap="wrap" flexDir="row" justifyContent={justifyContentMap[node.attrs?.style?.["text-align"]]}>
                    {node.children}
                </Box>);
        case "font":
        case "p":
            return (<Text key={key} fontSize={size} lineHeight={lineHeight} textAlign={node.attrs?.["text-align"]}>
                    {node.children}
                </Text>);
        case "b":
        case "strong":
        ...
        case "ul":
        case "ol":
            return (<Box key={key} w="100%">
                    {node.children}
                </Box>);
        case "li":
            return (<Box key={key} flexDir="row" _text={{
                    mr: 1,
                    w: 5,
                    textAlign: "right",
                }}>
        ...
        case "input":
            return (<Box key={key} borderWidth={1} rounded={5} px={1} mr={1} _text={{ color: "white", fontWeight: "bold" }} bg={node.attrs?.checked == '' ? 'primary' : "transparent"}>
                    ✓
                </Box>);
        case "br":
...
export const RenderHTML = ({ ...props }) => {
    return (<ScrollView _android={{ removeClippedSubviews: true }} bounces={false} overScrollMode="never" {...props}>
